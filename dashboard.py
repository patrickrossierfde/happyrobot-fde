...
# ==================== PERFORMANCE ====================
    ...
    st.markdown("Deeper insights into load popularity and revenue trends.")
    ...
    if calls_data and calls_data.get('calls'):
        ...
        if not df.empty:
            ...
            with col1:
                st.subheader("📦 Load ID Popularity")
                st.markdown("Which freight lanes are generating the most interest?")
                
                # Count calls per load
                load_counts = df['load_id'].value_counts().reset_index()
                load_counts.columns = ['Load ID', 'Total Inquiries']
            ...
            with col2:
                ...
                st.markdown("Cumulative revenue generated by the AI over time.")
                
                # Filter to only agreed deals
                agreed_df = df[df['outcome'] == 'agreed'].copy()
                
                if not agreed_df.empty:
                    # Sort by time and calculate cumulative sum
                    agreed_df = agreed_df.sort_values('created_at')
                    agreed_df['Cumulative Revenue'] = agreed_df['agreed_price'].astype(float).cumsum()
                    ...
                    fig2.update_traces(line_color='#2ECC71', marker=dict(size=8))
                    st.plotly_chart(fig2, use_container_width=True)
                else:
                    st.info("Book a load with the AI to see the revenue timeline!")
        else:
